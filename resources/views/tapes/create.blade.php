...
@section('title', 'Register Inventory Asset')
...
<style>
    ...
    /* Input Form Controls Overhaul - Fixed placeholder and type text blending properties */
...
<div class="container-fluid py-4">
    <div class="custom-card col-lg-8 mx-auto">
        <div class="custom-card-header d-flex justify-content-between align-items-center">
            <div>
                <h4>📼 Register Inventory Instance</h4>
                <p>Provision tracking configurations for a physical tape or disk unit item</p>
            </div>
            <a href="{{ route('tapes.index') }}" class="btn btn-outline-light btn-sm fw-semibold px-3">← Back</a>
        </div>
            ...
            <form method="POST" action="{{ route('tapes.store') }}">
                ...
                <div class="row mb-4">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="tape_number" id="tape_number" class="form-control form-control-custom @error('tape_number') is-invalid @enderror" value="{{ old('tape_number') }}" placeholder="Enter unique asset code (e.g., T-0011)" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="movie_id" id="movie_id" class="form-select form-control-custom text-white-custom @error('movie_id') is-invalid @enderror" required>
                            <option value="" class="text-white-custom">-- Assign Title Catalog Reference --</option>
                            @foreach($movies as $movie)
                                <option value="{{ $movie->id }}" class="text-white-custom" {{ old('movie_id') == $movie->id ? 'selected' : '' }}>{{ $movie->title }} ({{ $movie->movie_id }})</option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-4">
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="format" id="format" class="form-select form-control-custom text-white-custom @error('format') is-invalid @enderror" required>
                            <option value="" class="text-white-custom">-- Select Format --</option>
                            <option value="VHS" class="text-white-custom" {{ old('format') == 'VHS' ? 'selected' : '' }}>VHS Tape</option>
                            <option value="Beta" class="text-white-custom" {{ old('format') == 'Beta' ? 'selected' : '' }}>Beta Tape</option>
                            <option value="DVD" class="text-white-custom" {{ old('format') == 'DVD' ? 'selected' : '' }}>DVD Digital Disk</option>
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="shelf_location" id="shelf_location" class="form-control form-control-custom @error('shelf_location') is-invalid @enderror" value="{{ old('shelf_location') }}" placeholder="e.g., A-01, C-05">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="condition" id="condition" class="form-select form-control-custom text-white-custom @error('condition') is-invalid @enderror" required>
                            <option value="Good" class="text-white-custom" {{ old('condition', 'Good') == 'Good' ? 'selected' : '' }}>Good / Operational</option>
                            <option value="Fair" class="text-white-custom" {{ old('condition') == 'Fair' ? 'selected' : '' }}>Fair / Minor Scratches</option>
                            <option value="Poor" class="text-white-custom" {{ old('condition') == 'Poor' ? 'selected' : '' }}>Poor / Damage / Degrading</option>
                        </select>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top border-secondary border-opacity-10">
                    <a href="{{ route('tapes.index') }}" class="btn btn-outline-danger px-4 fw-semibold">Cancel</a>
                    <button type="submit" class="btn btn-primary px-4 fw-semibold">Save Unit Record</button>
                </div>
            </form>
        </div>
    </div>
