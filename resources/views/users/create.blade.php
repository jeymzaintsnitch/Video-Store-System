    ...
    <div class="custom-card col-lg-7 mx-auto">
        <div class="custom-card-header d-flex justify-content-between align-items-center">
            <div>
                ...
                <p>Provision credentials and assign system roles</p>
            </div>
            <a href="{{ route('users.index') }}" class="btn btn-outline-light btn-sm fw-semibold px-3">← Back to Index</a>
        </div>
            ...
            <form method="POST" action="{{ route('users.store') }}">
                ...
                <div class="row mb-3">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="name" id="name" class="form-control form-control-custom @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="e.g., John Doe" required>
                        @error('name') <span class="text-danger-custom">{{ $message }}</span> @enderror
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="email" class="form-label-custom">Email Address</label>
                        <input type="email" name="email" id="email" class="form-control form-control-custom @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="user@videostore.test" required>
                ...
                <div class="row mb-4">
                    <div class="col-md-6 mb-3">
                        <label for="password" class="form-label-custom">Account Password</label>
                        <input type="password" name="password" id="password" class="form-control form-control-custom @error('password') is-invalid @enderror" placeholder="Minimum 8 characters" required>
                ...
                <div class="mb-4">
                    ...
                    <select name="role" id="role" class="form-select form-control-custom text-white-custom @error('role') is-invalid @enderror" required>
                        <option value="" class="text-white-custom">-- Select Assigned Role --</option>
                        @foreach($roles as $role)
                            <option value="{{ $role->name }}" class="text-white-custom" {{ old('role') == $role->name ? 'selected' : '' }}>
                                {{ $role->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top border-secondary border-opacity-10">
                    <a href="{{ route('users.index') }}" class="btn btn-outline-danger px-4 fw-semibold">Cancel Setup</a>
                    <button type="submit" class="btn btn-primary px-4 fw-semibold">Provision Account</button>
                </div>
            </form>
        </div>
    </div>
