...
@section('title', 'Create System Role')
...
<div class="container-fluid py-4">
    <div class="custom-card col-lg-8 mx-auto">
        <div class="custom-card-header d-flex justify-content-between align-items-center">
            <div>
                <h4>🛡️ Construct New Role</h4>
                <p>Define authorization limits and grant system permissions</p>
            </div>
            <a href="{{ route('roles.index') }}" class="btn btn-outline-light btn-sm fw-semibold px-3">← Cancel Setup</a>
        </div>
        ...
        <div class="card-body p-4">
            <form method="POST" action="{{ route('roles.store') }}">
                ...
                <div class="mb-4 col-md-6">
                    ...
                    <input type="text" name="name" id="name" class="form-control form-control-custom @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="e.g., Manager, Supervisor, Auditor" required>
                    @error('name') <span class="text-danger-custom">{{ $message }}</span> @enderror
                </div>

                <h5 class="mb-3 mt-5 pb-2 border-bottom border-dark-custom fw-semibold text-light">Assign Privileges & Capabilities</h5>
                ...
                <div class="row">
                    ...
                    @empty
                        <div class="col-12">
                            <p class="text-light opacity-75">No system permissions are currently mapped in the database.</p>
                        </div>
                    @endforelse
                </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top border-dark-custom">
                    <button type="submit" class="btn btn-primary px-4 py-2 fw-semibold w-100 w-md-auto">Create Role Hierarchy</button>
                </div>
            </form>
        </div>
    </div>
