...
@section('title', 'Register New Genre')
...
<div class="container-fluid py-4">
    <div class="custom-card col-lg-6 mx-auto">
        <div class="custom-card-header d-flex justify-content-between align-items-center">
            <div>
                <h4>📑 Define New Genre</h4>
                <p>Add a new classification block for catalog organization</p>
            </div>
            <a href="{{ route('categories.index') }}" class="btn btn-outline-light btn-sm fw-semibold px-3">← Back to Index</a>
        </div>
        ...
        <div class="card-body p-4">
            <form method="POST" action="{{ route('categories.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="form-control form-control-custom @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="e.g., Suspense, Sci-Fi" required>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="5" class="form-control form-control-custom @error('description') is-invalid @enderror" placeholder="Define the parameters or rules for this specific genre...">{{ old('description') }}</textarea>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top border-secondary border-opacity-10">
                    <a href="{{ route('categories.index') }}" class="btn btn-outline-danger px-4 fw-semibold">Cancel Setup</a>
                    <button type="submit" class="btn btn-primary px-4 fw-semibold">Save Genre Block</button>
                </div>
            </form>
        </div>
    </div>
